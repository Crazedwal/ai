...
const QUEST_POOL = [
  ...
  { id: "long_chat_10", label: "Send 10 messages in one chat",    goal: 10, stat: "messagesInChat", reward: 35 },
  ...
  { id: "new_chat_5",   label: "Start 5 conversations",           goal: 5,  stat: "chatsCreated",   reward: 35 },
  // Plinko quests
  { id: "plinko_1",     label: "Play Plinko once",                goal: 1,  stat: "gamblesPlayed",  reward: 10 },
  { id: "plinko_5",     label: "Play Plinko 5 times",             goal: 5,  stat: "gamblesPlayed",  reward: 20 },
  { id: "plinko_10",    label: "Play Plinko 10 times",            goal: 10, stat: "gamblesPlayed",  reward: 35 },
  { id: "plinko_win",   label: "Profit from Plinko",              goal: 1,  stat: "gamblesWon",     reward: 25 },
  { id: "plinko_win3",  label: "Profit 3 times from Plinko",      goal: 3,  stat: "gamblesWon",     reward: 45 },
  { id: "plinko_lose",  label: "Take a loss in Plinko",           goal: 1,  stat: "gamblesLost",    reward: 10 },
  { id: "plinko_50",    label: "Drop 50 balls at once",           goal: 50, stat: "ballsDropped",   reward: 50 },
  { id: "plinko_500",   label: "Drop 500 balls total",            goal: 500,stat: "ballsDropped",   reward: 80 },
  // Token quests
  { id: "tokens_100",   label: "Reach 100 tokens",                goal: 100,stat: "peakBalance",    reward: 15 },
  { id: "tokens_500",   label: "Reach 500 tokens",                goal: 500,stat: "peakBalance",    reward: 30 },
  { id: "tokens_1k",    label: "Reach 1000 tokens",               goal: 1000,stat:"peakBalance",    reward: 60 },
]
...
export function QuestProvider({ children }) {
  ...
  const [stats, setStats] = useState(() => ({
    ...(loadSession().stats || {}),
    peakBalance: parseInt(localStorage.getItem("peakBalance") || "0", 10)
  }))
  ...
  // Check completions
  useEffect(() => {
    const currentPeak = parseInt(localStorage.getItem("peakBalance") || "0", 10)
    const mergedStats = { ...stats, peakBalance: currentPeak }
    const justDone = []
    for (const q of activeQuests) {
      if (!completed.includes(q.id) && (mergedStats[q.stat] || 0) >= q.goal) {
        justDone.push(q)
      }
    }
